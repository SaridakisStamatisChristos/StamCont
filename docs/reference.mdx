...
description: "Reference for the config.yaml compatibility format currently retained by StamCont."
---
<Warning>
This page documents an **inherited Continue-compatible configuration surface** that StamCont still supports. Names such as `continue.*`, `~/.continue`, and upstream-oriented examples may remain because they are live compatibility identifiers. They should not be read as claims that StamCont is the upstream Continue project.
</Warning>

## Introduction

StamCont currently retains the inherited `config.yaml` agent specification for compatibility.
